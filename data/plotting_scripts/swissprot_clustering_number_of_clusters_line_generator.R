...
p10 <- p10 + xlab("Number of Database Sequences") + ylab("Number of Clusters")
p10 <- p10 + scale_y_continuous(sec.axis = sec_axis(~./(as.integer(max(results_data$dataset_size)/100)), name = "Output Sequences[%]"))
p10 <- p10 + theme_classic()
ggsave(paste(file_prefix, "_number_of_database_sequences_vs_number_of_clusters.png", sep=""), plot=p10)